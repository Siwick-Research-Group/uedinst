...
class SC10Shutter(SerialBase):
    ...
    def __init__(self, port, **kwargs):
        kwargs.update({"port": port, "baudrate": 9600, "timeout": 2.0})
    ...
    def set_trigger_mode(self, mode):
        ...
        try:
            ...
            raise ValueError(
                "Trigger mode must be one of {}, not {}".format(
                    list(self.TriggerModes), mode
                )
            )
    ...
    def set_operating_mode(self, mode):
        ...
        try:
            ...
            raise ValueError(
                "Operating mode must be one of {}, not {}".format(
                    list(self.OperatingModes), mode
                )
            )
